...
export default function Projects({ onHide, isEditAllowed = false }: { onHide?: () => void; isEditAllowed?: boolean }) {
  ...
  const handleImageUpload = (file: File, type: 'new' | 'edit') => {
    ...
    if (file.size > 2.5 * 1024 * 1024) {
      if (type === 'new') {
        setFormError('Ukuran gambar terlalu besar! Harap unggah degan ukuran di bawah 2.5MB agar penyimpanan lancar.');
      } else {
        setEditFormError('Ukuran gambar terlalu besar! Harap unggah degan ukuran di bawah 2.5MB.');
      }
      return;
    }
  ...
  return (
    <div id="projects-showcase-section" className="bg-white rounded-[2rem] border border-gray-100 p-6 shadow-sm flex-1 min-w-[320px]">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4" id="projects-grid-layout">
        ...
        {filteredProjects.length === 0 && (
          <div className="md:col-span-2 py-12 text-center text-gray-400 bg-gray-50 rounded-2xl border border-dashed border-gray-200">
            ...
            {isEditAllowed && (
              <p className="text-[10px] text-gray-400 mt-1">Gunakan tombol 'Tambah Karya' untuk membuat hasil karya perdana Anda!</p>
            )}
          </div>
        )}
      </div>
      ...
      <AnimatePresence>
        {isAddModalOpen && (
          <div className="fixed inset-0 bg-black/50 backdrop-blur-xs flex items-center justify-center p-4 z-50" id="add-karya-modal-backdrop" onClick={() => setIsAddModalOpen(false)}>
            <motion.div
              initial={{ scale: 0.95, opacity: 0 }}
              animate={{ scale: 1, opacity: 1 }}
              exit={{ scale: 0.95, opacity: 0 }}
              onClick={(e) => e.stopPropagation()}
              className="bg-white rounded-[2rem] border border-gray-100 max-w-lg w-full p-6 text-left shadow-2xl relative max-h-[92vh] overflow-y-auto"
              ...
              <form onSubmit={handleAddProject} className="space-y-4">
                ...
                <div className="bg-neutral-50/70 p-4 rounded-2xl border border-dashed border-gray-200" id="image-upload-wrapper">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-3 items-start">
                    ...
                    <div className="flex flex-col gap-1.5">
                      <span className="text-[9px] font-bold text-gray-400 uppercase">Opsi B: Masukkan Link Gambar langsung</span>
                      <div className="relative">
                ...
                <div className="pt-3 border-t border-gray-100 flex gap-2">
                  <button
                    type="button"
                    onClick={() => setIsAddModalOpen(false)}
                    className="flex-1 bg-gray-50 hover:bg-gray-100 text-gray-600 text-xs font-semibold py-2.5 rounded-xl transition-all cursor-pointer text-center"
                  >
                    Batal
                  </button>
                  <button
      ...
      <AnimatePresence>
        {editingProject && (
          <div className="fixed inset-0 bg-black/50 backdrop-blur-xs flex items-center justify-center p-4 z-50" id="edit-karya-modal-backdrop" onClick={() => setEditingProject(null)}>
            <motion.div
              initial={{ scale: 0.95, opacity: 0 }}
              animate={{ scale: 1, opacity: 1 }}
              exit={{ scale: 0.95, opacity: 0 }}
